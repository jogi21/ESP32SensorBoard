...
char *PubMQTT_Temperatur = "Temperatur";
...
void Sensor()
{
    ...
    if ((err = dht11.read(&temperature, &humidity, NULL)) != SimpleDHTErrSuccess) 
    {
      ...
      display.setFont(ArialMT_Plain_16);
      display.setTextAlignment(TEXT_ALIGN_CENTER);
      display.drawString(64, 16, "Fehler");
      display.setTextAlignment(TEXT_ALIGN_CENTER);
      display.drawString(64, 32, "Kein Sensor!");
      display.setTextAlignment(TEXT_ALIGN_CENTER);
      display.drawString(64, 48, "I [mA] : 00.00!");

      
      numberString = String("Err");

    } else {

      numberString = String((float)temperature);

      display.setFont(ArialMT_Plain_16);
      display.setTextAlignment(TEXT_ALIGN_LEFT);
      display.drawString(0, 16, "Temp.:");
      display.setTextAlignment(TEXT_ALIGN_RIGHT);
      display.drawString(128, 16, numberString);

      display.setFont(ArialMT_Plain_16);
      display.setTextAlignment(TEXT_ALIGN_LEFT);
      display.drawString(0, 32, "I 0-20 [mA] :");
      display.setTextAlignment(TEXT_ALIGN_RIGHT);
      display.drawString(128, 32, String(((float)temperature + 20.0) / 80.0 * 20.0));

      display.setFont(ArialMT_Plain_16);
      display.setTextAlignment(TEXT_ALIGN_LEFT);
      display.drawString(0, 48, "I 4-20 [mA] :");
      display.setTextAlignment(TEXT_ALIGN_RIGHT);
      display.drawString(128, 48, String(4+((float)temperature + 20.0) / 80.0 * 16.0));

    }

  Publish2MQTT(PubMQTT_Temperatur, numberString);  
